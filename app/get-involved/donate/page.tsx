...
import mockData from '@/mockData/donations.json';
import { DonatePageProps, DonationOption, Program, DonorInfo, BankDetails, DonationFormData } from '@/app/types/types';
import DonationDialog from '@/app/components/PagesComponents/DonationDialog';
...
const DonatePage: React.FC<DonatePageProps> = ({ onBack }) => {
  ...
  const donationOptions: DonationOption[] = mockData.donationOptions;
  const programs: Program[] = mockData.programs;
  const bankDetails: BankDetails = mockData.bankDetails;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Open the donation dialog instead of processing payment
    setIsDialogOpen(true);
  };

  // ⭐ UPDATED: This is the main change you need to make
  const handleDialogComplete = () => {
    ...
    if (typeof window !== 'undefined') {
      sessionStorage.setItem('donationSuccess', JSON.stringify(donationData));
    }
    
    // Navigate to success page
    router.push('/get-involved/donate/success');
  };

  const getDonationFormData = (): DonationFormData => ({
    amount: getDonationAmount(),
  ...
  return (
    <>
            ...
            <form onSubmit={handleSubmit} className="p-8">
              ...
              <div className="text-center">
                ...
                <p className="text-sm text-gray-500 mt-4">
                  You will receive bank transfer details to complete your donation.
                </p>
              </div>
            </form>
          </div>
      ...
      <DonationDialog
        ...
        donationData={getDonationFormData()}
      />
    </>
  );
};
